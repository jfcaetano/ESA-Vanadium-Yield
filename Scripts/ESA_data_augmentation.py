...
# Define numeric variables for which we need to compute the constraints
...
# Ensure all numeric variables are of numeric type
...
# Loop over each lab and calculate min and max values for the numeric variables
...
# Convert the list of constraints into a DataFrame
...
# Apply constraints filter to data
...
def generate_synthetic_row_with_fixed_variation(lab_name, lab_constraints, numeric_variables, original_row, fixed_variation=0.1):
    synthetic_row = original_row.copy()  # Start by copying the original row
    ...
    for var in numeric_variables:
        ...
        if (var == 'Catalyst_quant_mmol' and lab_constraint['Catalyst_quant_mmol_min'] == 0) or \
           (var == 'Time_h' and lab_constraint['Time_h_min'] == 0):
            return None
...
# Add synthetic rows
num_synthetic_rows = 20  # per laboratory
fixed_variation = 0.1  # 10% variation
synthetic_data = []
for lab_name, lab_group in grouped_labs:
    for _ in range(num_synthetic_rows):
        original_row = lab_group.sample(n=1).iloc[0]  # Sample one original row from the group
...
# Function to generate rows with Yield = 0 by setting Yield, Time_h, and Catalyst_quant_mmol to 0
def generate_yield_zero_rows(data, catalyst_type_column, percentage=0.05):

    non_zero_yield_data = data[data['Yield'] != 0]
    sampled_data = non_zero_yield_data.groupby(catalyst_type_column).apply(lambda x: x.sample(frac=percentage, replace=True)).reset_index(drop=True)
    
    sampled_data['Yield'] = 0
    sampled_data['Time_h'] = 0
    sampled_data['Catalyst_quant_mmol'] = 0
    sampled_data['is_synthetic'] = 1
    
    return sampled_data

combined_data['Original_Cat_Structure'] = combined_data['Cat_Structure']
combined_data['Original_Lab'] = combined_data['Laboratory']


# Define the Catalyst-Type column and the percentage of data for Yield = 0
catalyst_type_column = 'Laboratory' 
yield_zero_data = generate_yield_zero_rows(combined_data, catalyst_type_column, percentage=0.05)
combined_data = pd.concat([combined_data, yield_zero_data], ignore_index=True)


# Bin the 'Yield' column
combined_data['Yield_bin'] = pd.cut(combined_data['Yield'], bins=10, labels=False, include_lowest=True)

# Determine the average number of points per bin
...
# Print underrepresented bins
print("Underrepresented bins: ", underrepresented_bins)
if not underrepresented_bins.empty:
    print(f"Generating synthetic rows for underrepresented bins: {underrepresented_bins}")

# Adjust sampling for bins with low counts (sampling with replacement)
def generate_synthetic_rows_for_underrepresented_bins(data, underrepresented_bins, num_synthetic_rows_per_bin=20, fixed_variation=0.1):
    ...
    for bin_num in underrepresented_bins.index:  # Ensure correct iteration over bin indices
        ...
        for _ in range(num_synthetic_rows_per_bin):
            # Sample with replacement to avoid empty bin issues
            original_row = bin_data.sample(n=1, replace=True).iloc[0]
            lab_name = original_row['Laboratory']
            
            # Generate a synthetic row with fixed variation
            ...
            if synthetic_row is not None:
                ...
                print(f"Could not generate synthetic row for bin {bin_num}, lab {lab_name}")
...
# Ensure synthetic data is generated and not empty
if not synthetic_underrepresented_data.empty:
    print("Synthetic rows generated for underrepresented bins.")
...
# Fit and transform the one-hot encoding on the combined data
